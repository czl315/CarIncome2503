package work.netCar.dao;

import netCar.dao.MyBatisUtils;
...
public class CarIncomeDao {
    ...
    public static int insert(CarIncome entity) {
        ...
        int rs = 0;
        try {
            rs = session.insert("work.netCar.dao.mapper.CarIncomeMapper.insert", entity);
            session.commit();
        } finally {
            session.close();
        }
    ...
    public static List<CarIncome> findListByCondition(CarIncome condition) {
        ...
        try {
            rs = session.selectList("work.netCar.dao.mapper.CarIncomeMapper.findListByCondition", condition);
            session.commit();
        } catch (Exception e) {
//            e.printStackTrace();
    ...
    public static int update(CarIncome entity) {
        ...
        int rs = 0;
        try {
            rs = session.update("work.netCar.dao.mapper.CarIncomeMapper.update", entity);
            session.commit();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
