...
//        findListTongJj("2021-09-01","2021-09-31");//查询-统计数据
     ...
     * @param startNum
     */
    public static void updateTodayStCom(String date, int startNum) {
...
//                    System.out.println(funcName + "数据异常:" + JSON.toJSONString(stockInfo));
                    continue;
                }
...
//                            System.out.println(qDate + "没有查询到！");
    ...
    public static int updateConception(String date, int startNum) {
        ...
        for (RankBizDataDiff bk : bkListMust) {
            ...
            stBizCountTemp++;
            List<RankStockCommpanyDb> stockList = BizService.listRankStockByBiz(NUM_MAX_999, banKuaiCode);
            int stockCount = stockList != null ? stockList.size() : 0;
            //查询数据库中已存在概念的个数
...
//            System.out.println();
            ...
            for (RankStockCommpanyDb stockInfo : stockList) {
                ...
                if (stCode.startsWith("5") || stCode.startsWith("6") || stCode.startsWith("90")) {
                    urlParam.append("code=SH").append(stCode);
                } else if (stCode.startsWith("4") || stCode.startsWith("8") || stCode.startsWith("920")) {
                    //920开头的股票为北交所股票
                    urlParam.append("code=BJ").append(stCode);
                } else {
...
//        sb.append("\t").append(",当前：").append( DateUtil.getToday(DateUtil.YYYY_MM_DD_HH_MM_SS));
    ...
    public static void addTodayStComByExistCount(String date, int countNotNullLimit) {
        ...
        if (countNotNull == null || countNotNull < countNotNullLimit) {
            ...
            System.out.println(funcName + "查询指定日期数据库中已存在的个数:限定个数【" + countNotNull + ":" + countNotNullLimit + "】,数据已存在，无需新增");
        }
