...
//        }
     ...
     * @param date
     */
    public static void updateLatestDayAdrStock(CondEtfAdrCount condition, String date) {
        String methodName = "更新最近交易日的涨幅(股票)-";
        boolean isShowLog = true;
        long begTime = System.currentTimeMillis();
...
//                    System.out.println("不更新非主板股票：" + zqdm + "," + zqmc);
                ...
                EtfAdrCount entity = new EtfAdrCount();
                entity.setDate(date);
                entity.setF12(stockCommpanyDb.getF12());

                //显示指定日期最近3个K线交易日的涨跌
                int days = 3;
                List<Kline> klineListDays = SseService.klineExchangeStock(zqdm, 4, CYCLE_TYPE_DAY);
                if (klineListDays == null) {
                    continue;
                }
                for (Kline klineListDay : klineListDays) {
...
//                System.out.println(methodName + "如果更新失败，可能是没有插入，执行一次插入操作：" + rsCountInsert);
     ...
     * @param channel         渠道
     */
    public static void saveOrUpdateLastDayStock(CondEtfAdrCount condition, String date, boolean isUpdateNoToday, String channel) {
        long begTime = System.currentTimeMillis();
        boolean isShowLog = true;
        String methodName = "保存或更新涨幅次数(股票)";

        BigDecimal mvMin = condition.getMvMin();//NUM_YI_1000
        BigDecimal mvMax = condition.getMvMax();
        String spBizName = null;
...
//                System.out.println(methodName + "如果更新失败，可能是没有插入，执行一次插入操作：" + rsCountInsert);
            ...
            if (isShowLog) {
                System.out.println(methodName + "，用时：" + (System.currentTimeMillis() - begTime) / 1000 + ",更新成功：" + rsCountUpdate + ",更新失败：" + rsCountUpdateFail + ",插入成功：：" + rsCountInsert + ",类型：：" + bizName);
            }
...
//        updateAdrSumByBiz(date, DB_STOCK_ADR_COUNT_ADR_UP_SUM_1_1, dateList, etfList, httpKlineApiType);
     ...
     * @param date date
     */
    public static void updateUpSumOrder(String date, String channel) {
     ...
     * @param date
     */
    public static void updateUpSumOrderStock(String date, String channel) {
        List<RankBizDataDiff> bizList = StockService.listBiz(NUM_MAX_99);//查询业务列表
        for (RankBizDataDiff rankBizDataDiff : bizList) {
            String bizName = rankBizDataDiff.getF14();
            //更新-上涨之和排序
            ...
            updateAdrSumOrderStatByBiz(date, bizName, channel);
...
//                System.out.println("今日未涨：" + JSON.toJSONString(entity));
    ...
    /**
     * 更新上涨之和:1、计算正增长。2、如果上涨之和高于限定值，上涨之和设置为限定值。
     * 3、跌幅累计
     *
     * @param date
     * @param etfList
...
//                    }
     ...
     * @return
     */
    private static int updateAdrSumStock(String date, String spBizName) {
        long begTime = System.currentTimeMillis();
        boolean isShowLog = true;
...
//                    System.out.println("(" + zqmc + ")市值判定不通过(" + (++countMvCheckNo) + ")：" + mv.divide(NUM_YI_1).setScale(0, BigDecimal.ROUND_HALF_UP));
                ...
                List<Kline> klines60 = SseService.klineExchangeStock(zqdm, DAYS_INT_61, CYCLE_TYPE_DAY);//查询最近61天k线
                EtfAdrCount entity = new EtfAdrCount();
                entity.setF12(zqdm);
                entity.setDate(date);
...
//                System.out.println("当前页查询个数：" + curPageEtfList.size());
     ...
     * @return
     */
    public static List<RankBizDataDiff> listEtfListLastDayByMarketValue(BigDecimal minMv, BigDecimal maxMv, String
            spTypeName) {
        boolean isShowLog = true;
...
//            System.out.println("(" + zqmc + ")市值最低判定最低不通过：" + mv + ":" + minMv);
     ...
     * @param bizName 业务名称
     */
    private static int updateAdrSumOrderStatByBiz(String date, String bizName, String channel) {
